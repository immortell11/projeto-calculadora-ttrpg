...
O programa pede pelo valor dos pontos de vida do personagem, que normalmente variam entre 10 e 18. Após isso, ele pede pelo valor de vigor, que é um número somado aos pontos de vida toda vez que um personagem sobe de nível.

Finalmente, ele pede pelo nível do personagem, que vai de 1 a 20. Nessa operação matemática, o nível funciona como um multiplicador para o vigor. A calculadora realiza a operação e te entrega o valor final dos pontos de vida do personagem.

O processo se repete com o mesmo cálculo para os seus pontos de energia e o seu fôlego, que substituem os pontos de vida e o vigor, respectivamente.

As próximas informações calculadas pelo programa são as magias e habilidades do seu personagem. Os cálculos são baseados no seu nível, cujo valor foi pedido anteriormente na operação dos pontos de vida.
...
- Pontos de vida (PV) = pv + vigor + (vigor * (nível - 1))

- Pontos de energia (PE) = pe + fôlego + (fôlego * (nível - 1))

- Magias = 3 + (nível - 1)

- Habilidades = nível / 2 (arredondado para baixo)
